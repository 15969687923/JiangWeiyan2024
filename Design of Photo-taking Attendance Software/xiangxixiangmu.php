...
<ul class="mui-table-view">
	<?php
			$sql = "select touxiang,yonghuming,shibiezhuangtai from xiangxi where xiangmuhao = '$q'";
			$x = $conn->query($sql);
			while($row = $x->fetch_assoc()){
				echo "<li class='mui-table-view-cell mui-media'>
        		<a href='javascript:;'>
            	<img class='mui-media-object mui-pull-left' src='{$row['touxiang']}'>
            	<div class='mui-media-body'>
                {$row['yonghuming']}
                <p class='mui-ellipsis'>{$row['shibiezhuangtai']}</p>
            	</div>
        		</a>
    			</li>";
			}
...
<div class="an">
<form action="shanchuxiangmu.php" method="post">
	<input type="hidden" name='chuan' value="<?php  echo $_GET['x'];?>">
<button type="submit" class="mui-btn mui-btn-danger an1">删除项目</button>
</form>
<button type="button" class="mui-btn mui-btn-warning an1" onclick="window.location.href='chongxinfabu.php?x=<?php  echo $_GET['x'];?>'">重新发布</button>
</div>
</div>
	</body>
