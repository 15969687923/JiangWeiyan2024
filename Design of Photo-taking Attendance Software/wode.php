...
<html>
	...
	<body>
		...
		<header class="mui-bar mui-bar-nav mui-bar-transparent biao11">
					<h1 class="mui-title biao12">我管理的项目</h1>
	...
	<ul class="mui-table-view">
  	...
  	<?php
		...
		exit(0);
			...
			$sql = "select name,xiangmuhao,kaishi,tupian1 from xiangmujl where shoujihao = '$_SESSION[dianhuahao]'";
			$x = $conn->query($sql);
			$number = mysqli_num_rows($x);
			if($number==NULL){
			}else{
			while($row = $x->fetch_assoc()){
				echo "<li class='mui-table-view-cell mui-media'>
				<a href='xiangxixiangmu.php?x={$row['xiangmuhao']}'>
